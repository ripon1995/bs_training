...
class RetrofitApiClientTypeCode {
    companion object {
        private const val Type_Code_Base_Url = "https://jsonplaceholder.typicode.com/"
        private var retrofit: Retrofit? = null
        private val gson: Gson = GsonBuilder().setLenient().create()

        fun getClientTypeCode(): Retrofit? {
            if (retrofit == null) {
                synchronized(RetrofitApiClientHotelData::class.java)
                {
                    if (retrofit == null) {
                        retrofit =
                            Retrofit.Builder().baseUrl(Type_Code_Base_Url).client(getLogger())
                                .client(RetrofitApiClientHotelData.getLogger())
